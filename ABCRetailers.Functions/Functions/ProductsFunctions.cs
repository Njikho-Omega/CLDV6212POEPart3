...
public class ProductsFunctions
{
    ...
    [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "products")] HttpRequestData req)
    {
        ...
        {
            Console.WriteLine($"Error in Products_Create: {ex}");
            return await HttpJson.Bad(req, $"Error creating product: {ex.Message}");
        }
    }
    ...
    [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "products/{id}/upload-image")] HttpRequestData req,
    string id)
    {
        try
        {
            Console.WriteLine("=== ProductImage_Upload Started ===");

            var body = await HttpJson.ReadAsync<Dictionary<string, string>>(req);
            var imageBase64 = body?["imageData"] ?? "";

            if (string.IsNullOrEmpty(imageBase64))
            {
                return await HttpJson.Bad(req, "No image data provided");
            }
            ...
            var table = new TableClient(_conn, _table);
            var productResp = await table.GetEntityAsync<ProductEntity>("Product", id);
            var product = productResp.Value;
            product.ImageUrl = imageUrl;
            await table.UpdateEntityAsync(product, product.ETag, TableUpdateMode.Replace);

            return await HttpJson.Ok(req, new { imageUrl });
        }
        catch (Exception ex)
        {
            Console.WriteLine($"!!! ProductImage_Upload ERROR: {ex}");
            return await HttpJson.Bad(req, $"Image upload failed: {ex.Message}");
        }
    }
}
