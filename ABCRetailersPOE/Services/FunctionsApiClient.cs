...
namespace ABCRetailersPOE.Services
{
    public class FunctionsApiClient : IFunctionsApi
    {
        ...
        public async Task UpdateOrderStatusAsync(string id, string newStatus)
        {
            try
            {
                System.Diagnostics.Debug.WriteLine($"=== API: Updating order {id} to status {newStatus} ===");

                var payload = new { Status = newStatus };
                var jsonContent = JsonBody(payload);

                var jsonString = await jsonContent.ReadAsStringAsync();
                System.Diagnostics.Debug.WriteLine($"=== SENDING JSON: {jsonString} ===");

                var response = await _http.PatchAsync($"{OrdersRoute}/{id}/status", jsonContent);

                if (!response.IsSuccessStatusCode)
                {
                    var errorContent = await response.Content.ReadAsStringAsync();
                    System.Diagnostics.Debug.WriteLine($"=== API ERROR: {response.StatusCode} - {errorContent} ===");
                    throw new HttpRequestException($"Failed to update order status. Status: {response.StatusCode}, Error: {errorContent}");
                }

                System.Diagnostics.Debug.WriteLine($"=== API: Successfully updated order status ===");
            }
            catch (Exception ex)
            {
        ...
        {
            var dtos = await ReadJsonAsync<List<Customer>>(await _http.GetAsync(CustomersRoute));
            return dtos; // no role filtering
        }

        // Get user by username (any role)
